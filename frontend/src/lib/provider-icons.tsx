...
// lich does not depend on that package's ~500-package tree (antd, shiki, …) for
// three icons. Crush has no lobehub mark yet and borrows a lucide glyph; shell
// shows a terminal. All render monochrome (currentColor), inheriting the
...
export function ProviderIcon({ kind, size = 16 }: { kind: SessionKind; size?: number }) {
  const d = PATHS[kind]
  if (d) {
    return <BrandIcon d={d} size={size} />
  }
  const Fallback = FALLBACK[kind] ?? Terminal
